...
build_dynamic_universe_details = getattr(UB, "build_dynamic_universe_details", None)
...
def run_night_research():
    ...
    details = build_dynamic_universe_details(target_size=int(getattr(CFG, "RESEARCH_UNIVERSE_SIZE", 20))) if callable(build_dynamic_universe_details) else {"selected": list(UB.build_dynamic_universe(target_size=int(getattr(CFG, "RESEARCH_UNIVERSE_SIZE", 20))) or [])}
    ...
    UB.save_universe(uni, live_path)
...
def refresh_top_movers_from_research():
    ...
    if bool(getattr(CFG, "INTRADAY_DYNAMIC_REFRESH", True)) and _is_heavy_refresh_due():
        details = build_dynamic_universe_details(target_size=target) if callable(build_dynamic_universe_details) else {"selected": list(UB.build_dynamic_universe(target_size=target) or [])}

...
def run_night_job():
    ...
    details = UB.build_dynamic_universe_details(target_size=int(getattr(CFG, "RESEARCH_UNIVERSE_SIZE", 20))) if hasattr(UB, "build_dynamic_universe_details") else {"selected": list(UB.build_dynamic_universe(target_size=int(getattr(CFG, "RESEARCH_UNIVERSE_SIZE", 20))) or [])}
    ...
    write_path = UB.save_universe(selected, live_path) if selected else live_path

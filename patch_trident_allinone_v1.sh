...
BK="$ROOT/backups/$TS-allinone"
mkdir -p "$BK"

echo "== Backing up current files to: $BK =="
for f in trading_cycle.py bot.py config.py; do
  if [ -f "$ROOT/$f" ]; then
    cp -a "$ROOT/$f" "$BK/$f"
  fi
done
...
sudo systemctl stop trident 2>/dev/null || true

echo "== Writing clean trading_cycle.py (All-in-one v1) =="
...
echo "== Compile check =="
./venv/bin/python -m py_compile "$ROOT/trading_cycle.py"

echo "== Restarting service =="
sudo systemctl start trident

echo "DONE ✅ All-in-one patch applied."
echo
echo "LIVE LOGS:"
echo "  sudo journalctl -u trident -f -o cat | egrep \"ENGINE|MODE|SYNC|SCAN|ORDER|EXIT|LOCK|ERROR|WARN\""
